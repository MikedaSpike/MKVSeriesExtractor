...
Public Class MakeMKVSerieExtractor
    ...
    Private logFile As String = Path.Combine(Application.StartupPath, exeName & ".log")
...
#Region "Control Handlers"
    ...
    Private Sub btnStart_Click(sender As Object, e As EventArgs) Handles btnStart.Click
        ...
        Dim tempExtractFolder As String = String.Empty
        ...
        Try
            ...
            For i As Integer = 0 To totalItems - 1
                ...
                tempExtractFolder = Path.Combine(outputPath, $"temp_extract")
            ...
            If Not Extractor.CancelRequested Then
                ...
                Application.DoEvents()
                Try
                    If Directory.Exists(tempExtractFolder) Then
                        Directory.Delete(tempExtractFolder, recursive:=True)
                        AppendToLog($"TempFolder [{tempExtractFolder}] deleted successfully.")
                    End If
                Catch ex As Exception
                    AppendTxtToScreen($"❌ Error deleting TempFolder [{tempExtractFolder}]: {ex.Message}")
                End Try
                Thread.Sleep(1000)
            Else
...
#End Region
    ...
    ''' <summary>
    ''' Ensures that the specified INI file exists. If it does not, creates it with default [Settings] content.
    ''' </summary>
    ''' <param name="iniPath">The full file path to the INI file.</param>

    Sub EnsureIniFileExists(iniPath As String)
        If Not File.Exists(iniPath) Then
            Dim sb As New StringBuilder()
            sb.AppendLine("[Settings]")
            sb.AppendLine("InputFolder=")
            sb.AppendLine("OutputFolder=")
            sb.AppendLine("TitleList=")
            sb.AppendLine("MakeMKVPath=" & If(String.IsNullOrWhiteSpace(txtMakeMKVPath.Text), "", txtMakeMKVPath.Text))
            sb.AppendLine("ShowName=")
            sb.AppendLine("EpisodeDuration=0")
